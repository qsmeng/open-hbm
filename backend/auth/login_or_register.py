...
async def login(data: dict):
    ...
    with get_db_connection() as conn, conn.cursor(dictionary=True) as cursor:
        ...
        if user and bcrypt.checkpw(password.encode('utf-8'), user['password_hash'].encode('utf-8')):
            ...
            logger.info(f"User {username} logged in successfully, JWT token generated")
...
async def forgot_password(email: str):
    ...
    try:
        ...
        user = await conn.fetchrow("SELECT * FROM user_base_info WHERE email = $1", email)
        ...
        await conn.execute(
            "UPDATE user_base_info SET reset_token = $1, reset_token_expiry = $2 WHERE email = $3",
